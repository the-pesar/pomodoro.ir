<template>
  <ClientOnly>
    <main class="pmdr w-full min-h-screen">
      <div class="mx-auto w-12/12 md:w-10/12 lg:w-7/12 xl:w-5/12 2xl:w-4/12 py-5 px-3 md:px-10 ">
        <section
          class="relative bg-glass text-center p-5 text-lg text-white rounded-lg"
        >
          <span>{{ tasks.find((v) => v.selected)?.name ?? "هنوز هیچ کاری انتخاب نشده!" }}</span>
        </section>
        ...
        >
          ...
          <div class="py-10">
            ...
            <template v-else>
              <button
                class="timer-action bg-white w-[180px] h-[60px] text-xl font-['Vazirmatn'] rounded-lg mx-5"
                @click="restTimer"
              >
                دوباره
              </button>
